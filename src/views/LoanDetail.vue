	...
	<div>
		...
		<div class="content clearfix">
			<div class="detail-left">
				<div class="detail-left-title">{{loanInfo.productName}}（{{loanInfo.productNo}}期）</div>
				<ul class="detail-left-number">
					<li>
						<span>历史年化收益率</span>
						<p><b>{{loanInfo.rate}}</b>%</p>
						<span>历史年化收益率</span>
					</li>
					<li>
						<span>募集金额（元）</span>
						<p><b>{{loanInfo.productMoney}}</b>元</p>
						<span>募集中&nbsp;&nbsp;剩余募集金额{{loanInfo.leftProductMoney}}元</span>
					</li>
					<li>
						<span>投资周期</span>
						<!-- 需要处理 天 月 -->
						<p><b>{{loanInfo.cycle}}</b>个月</p>
					</li>
			...
			<div class="detail-right">
				...
				<div class="detail-right-mode">
					...
					<h3 class="detail-right-mode-title">预计利息收入{{incomeMoney}}（元）</h3>
					<form action="" id="number_submit">
						<p>请在下方输入投资金额</p>
						<input type="text" placeholder="请输入投资金额，应为100元整倍数" v-model="investMoney" @input="checkMoney"
							class="number-money">
		...
		methods: {
			...
			checkMoney() {
				...
				if(parseInt(this.investMoney) > parseInt(this.loanInfo.bidMaxLimit)){
					this.errorText = "投资金额至多"+this.loanInfo.bidMinLimit
					return
				}
				this.errorText = ""
				...
				if(this.loanInfo.productType==0){
					/* 新手宝  天*/
					this.incomeMoney=parseInt(this.investMoney)*dateRate*parseInt(this.loanInfo.cycle)
				}else{
					/* 优选 散标  月  30*/
					this.incomeMoney=parseInt(this.investMoney)*dateRate*(parseInt(this.loanInfo.cycle)*30)
				}
			...
			userInvest(){
				...
				/* 发送请求 */
				let userId =JSON.parse(window.sessionStorage.getItem("userInfo")).uid;
				let bidInfo={
					uid:userId,
					loanId:parseInt(this.loanId),
					bidMoney:this.investMoney
				}
				this.axios.post("/api/v1/loan/invest",bidInfo).then(resp=>{
			...
			detailLoad(){
				...
				this.axios.get('/api/v1/product/detail', {
					params: {
						lid: this.loanId
					}
				}).then(resp => {
